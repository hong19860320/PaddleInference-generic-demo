...
void process(const std::string &model_dir,
  ...
  for (size_t i = 0; i < sample_count; i++) {
    ...
    // Postprocess
    start = get_current_us();
    auto heatmap_tensor = predictor->GetOutputHandle(output_names[0]);
    auto heatmap_shape = heatmap_tensor->shape();
    auto heatmap_size = shape_production(heatmap_shape);
    if (heatmap_size < 6) {
      printf("[WARNING] No object detected.\n");
    }
    std::vector<float> heatmap_data(heatmap_size);
    heatmap_tensor->CopyToCpu(heatmap_data.data());
    auto index_tensor = predictor->GetOutputHandle(output_names[1]);
    auto index_shape = index_tensor->shape();
    auto index_size = shape_production(index_shape);
    std::vector<int64_t> index_data(index_size);
    index_tensor->CopyToCpu(index_data.data());
    ...
    get_final_preds(heatmap_data.data(),
                    heatmap_shape,
                    index_data.data(),
                    center,
                    scale,
